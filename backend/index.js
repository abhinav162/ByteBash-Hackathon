...
app.get("/product/:productId", async (req, res) => {
    Product.find({ _id: req.params.productId }).then((data, err) => {
        if (err) {
            return res.status(500).json({ message: 'Internal Server Error' })
        }
        if (data) {
...
// add address
app.post('/add-address', checkToken, async (req, res) => {
    const user_id = req.user_id;
...
app.get('/get-addresses', checkToken, async (req, res) => {
    const user_id = req.user_id;
...
// delete address
app.delete('/delete-address/:addressId', checkToken, async (req, res) => {
    const user_id = req.user_id;
